...
class custompage {
    ...
    public function createpage($data) {
        global $DB;
        return $DB->insert_record('local_discounts', $data);
    }
    ...
    public function updatepage($data) {
        global $DB;
        return $DB->update_record('local_discounts', $data);
    }
    ...
    public static function load($id, $editor = false) {
        ...
        $data = new \stdClass();
        if (intval($id) > 0) {
            $data = $DB->get_record_sql("SELECT * FROM {local_discounts} WHERE id=? LIMIT 1", array(intval($id)));
        } else {
            ...
            $url = '%' . end($parts) . '%';
            $page = $DB->get_record_sql("SELECT * FROM {local_discounts} limit 1");
        ...
        if (!$editor) {
            $data->emailcontent = file_rewrite_pluginfile_urls($data->emailcontent, 'pluginfile.php',
                $context->id, 'local_discounts', 'emailcontent', null);
        }
