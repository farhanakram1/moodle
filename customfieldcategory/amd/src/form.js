 ...
 *
 * @module     core_customfieldcategory/form
...
define(['jquery', 'core/str', 'core/notification', 'core/ajax', 'core/templates', 'core/sortable_list', 'core/inplace_editable'],
    ...
    var confirmDelete = function(id, type, component, area, itemid) {
        Str.get_strings([
            {'key': 'confirm'},
            {'key': 'confirmdelete' + type, component: 'core_customfieldcategory'},
            {'key': 'yes'},
            {'key': 'no'},
        ]).done(function(s) {
            Notification.confirm(s[0], s[1], s[2], s[3], function() {
                var func = (type === 'field') ? 'core_customfieldcategory_delete_field' : 'core_customfieldcategory_delete_category';
                Ajax.call([
                    {methodname: func, args: {id: id}},
                    {methodname: 'core_customfieldcategory_reload_template', args: {component: component, area: area, itemid: itemid}}
                ])[1].then(function(response) {
                    return Templates.render('core_customfieldcategory/list', response);
    ...
    var createNewCategory = function(component, area, itemid) {
        var promises = Ajax.call([
                {methodname: 'core_customfieldcategory_create_category', args: {component: component, area: area, itemid: itemid}},
                {methodname: 'core_customfieldcategory_reload_template', args: {component: component, area: area, itemid: itemid}}
            ]),
            categoryid;
        ...
        init: function() {
            ...
            var categoryName = function(element) {
                return element
                    ...
                    .find('[data-inplaceeditable][data-itemtype=category][data-component=core_customfieldcategory]')
                    .attr('data-value');
            };
            ...
            $('[data-category-id]').on('sortablelist-drop', function(evt, info) {
                if (info.positionChanged) {
                    var promises = Ajax.call([
                        {
                            methodname: 'core_customfieldcategory_move_category',
                            args: {
            ...
            $('[data-field-name]').on('sortablelist-drop', function(evt, info) {
                ...
                if (info.positionChanged) {
                    var promises = Ajax.call([
                        {
                            methodname: 'core_customfieldcategory_move_field',
                            args: {
                ...
                Str.get_string('therearenofields', 'core_customfieldcategory').then(function(s) {
