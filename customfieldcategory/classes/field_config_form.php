...
namespace core_customfieldcategory;
...
class field_config_form extends \moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('header', '_commonsettings', get_string('commonsettings', 'core_customfieldcategory'));

        $mform->addElement('text', 'name', get_string('fieldname', 'core_customfieldcategory'), 'size="50"');
        ...
        $mform->addElement('text', 'shortname', get_string('fieldshortname', 'core_customfieldcategory'), 'size=20');
        $mform->addHelpButton('shortname', 'shortname', 'core_customfieldcategory');
        $mform->addRule('shortname', null, 'required', null, 'client');
        $mform->setType('shortname', PARAM_TEXT);

        $desceditoroptions = $handler->get_description_text_options();
        $mform->addElement('editor', 'description_editor', get_string('description', 'core_customfieldcategory'), null, $desceditoroptions);
        $mform->addHelpButton('description_editor', 'description', 'core_customfieldcategory');

        // If field is required.
        $mform->addElement('selectyesno', 'configdata[required]', get_string('isfieldrequired', 'core_customfieldcategory'));
        $mform->addHelpButton('configdata[required]', 'isfieldrequired', 'core_customfieldcategory');
        $mform->setType('configdata[required]', PARAM_BOOL);

        // If field data is unique.
        $mform->addElement('selectyesno', 'configdata[uniquevalues]', get_string('isdataunique', 'core_customfieldcategory'));
        $mform->addHelpButton('configdata[uniquevalues]', 'isdataunique', 'core_customfieldcategory');
    ...
    public function validation($data, $files = array()) {
        ...
        if (!preg_match('/^[a-z0-9_]+$/', $data['shortname'])) {
            ...
            $errors['shortname'] = get_string('invalidshortnameerror', 'core_customfieldcategory');
            ...
            $errors['shortname'] = get_string('formfieldcheckshortname', 'core_customfieldcategory');
        }
