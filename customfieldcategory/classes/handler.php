...
use core_customfieldcategory\output\field_data;
use stdClass;
 ...
 * <COMPONENT_OR_PLUGIN>\customfield\<AREA>_handler extends \core_customfieldcategory\handler
 *
 * To initiate a class use an appropriate static method:
 * - <handlerclass>::create - to create an instance of a known handler
 * - \core_customfieldcategory\handler::get_handler - to create an instance of a handler for given component/area/itemid
 *
 * Also handler is automatically created when the following methods are called:
 * - \core_customfieldcategory\api::get_field($fieldid)
 * - \core_customfieldcategory\api::get_category($categoryid)
 *
 * @package core_customfieldcategory
 * @copyright 2018 David Matamoros <davidmc@moodle.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
abstract class handler {
    ...
    public static function get_handler(string $component, string $area, int $itemid = 0) : handler {
        ...
        throw new \moodle_exception('unknownhandler', 'core_customfieldcategory', (object)$a);
    }
    ...
    protected function generate_category_name($suffix = 0) : string {
        if ($suffix) {
            return get_string('otherfieldsn', 'core_customfieldcategory', $suffix);
        } else {
            return get_string('otherfields', 'core_customfieldcategory');
        }
    }
    ...
    protected function validate_category(category_controller $category) : category_controller {
        ...
        if (!array_key_exists($category->get('id'), $categories)) {
            throw new \moodle_exception('categorynotfound', 'core_customfieldcategory');
        }
    ...
    protected function validate_field(field_controller $field) : field_controller {
        if (!array_key_exists($field->get('categoryid'), $this->get_categories_with_fields())) {
            throw new \moodle_exception('fieldnotfound', 'core_customfieldcategory');
        }
     ...
     * The caller must check access to the instance itself before invoking this method
     *
     * The result is an array of \core_customfieldcategory\output\field_data objects
     *
     * @param int $instanceid
     * @param bool $returnall
     * @return \core_customfieldcategory\output\field_data[]
     */
        ...
        foreach ($fieldswithdata as $data) {
            ...
            if (strlen($field->description)) {
                ...
                $value = file_rewrite_pluginfile_urls($field->description, 'pluginfile.php',
                    $context->id, 'core_customfieldcategory', 'description', $field->id);
    ...
    public function setup_edit_page(field_controller $field) : string {
        ...
        if ($field->get('id')) {
            $title = get_string('editingfield', 'core_customfieldcategory',
                $field->get_formatted_name());
        } else {
            $title = get_string('addingnewcustomfield', 'core_customfieldcategory', $typestr);
        }
